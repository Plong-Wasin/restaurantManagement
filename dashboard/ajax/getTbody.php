...
?>
    ...
    if ($_POST["data"] == "option1")
        $sql .= "DATE(check_in.paid_timestamp)=CURDATE() GROUP BY order_list.food_name,order_list.food_price ORDER BY order_list.food_name";
    elseif ($_POST["data"] == "option2")
        $sql .= "DATE(check_in.paid_timestamp) BETWEEN DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND CURDATE() GROUP BY order_list.food_name,order_list.food_price ORDER BY order_list.food_name";
    elseif ($_POST["data"] == "option3")
        $sql .= "MONTH(check_in.paid_timestamp) = MONTH(CURRENT_DATE()) AND YEAR(check_in.paid_timestamp) = YEAR(CURRENT_DATE()) GROUP BY order_list.food_name,order_list.food_price ORDER BY order_list.food_name";
    elseif ($_POST["data"] == "option4") {
        $formDate = $_POST["form"];
        $toDate = $_POST["to"];
        if ($formDate != "" && $toDate != "") {
